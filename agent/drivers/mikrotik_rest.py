"""Driver MikroTik via REST API di RouterOS 7.

Legge dal router le tabelle che una scansione dalla LAN non puo' vedere: lease
DHCP, registration-table WiFi con il segnale, bridge host table e vicini
MNDP/CDP/LLDP. E' la stessa raccolta che fa lo script incollato sul router,
fatta pero' da remoto: cosi' il MikroTik non ha piu' bisogno di uno script.

Requisiti sul router:

    /ip service enable www-ssl          (o www, ma in chiaro sulla LAN)
    /user add name=netmonitor group=read password=...

Il gruppo ``read`` basta: questo driver non scrive nulla.

Sul certificato: RouterOS presenta un certificato autofirmato, quindi la
verifica TLS e' disattivata per impostazione predefinita. E' una scelta
consapevole e limitata - la connessione resta cifrata, e la controparte e' un
indirizzo IP sulla LAN locale, non un host su Internet. Chi vuole la verifica
importa una CA sul router e mette ROUTER_VERIFY_TLS=true.
"""
